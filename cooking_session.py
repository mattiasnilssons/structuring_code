...
def remove_spikes(df_raw, min_size_of_spikes=1):
    # (1) Why range(100)? = There are more than 50 recordings in some of these spikes.
    # (2) Why not drop rows? = We could lose the dates of when some of these
    # meters were online.
    # (3) Why a min_size_of_spikes? = if the min_size is approx. 1 kWh, the timestamp issues gets visable.
    # Also, we don't lose any small energy increases.
    for _ in range(100):
...
def cooking_event(
    ...
    # (a): array of when a load is applied
    df_processed.loc[(
        (
            (df_processed.energy.diff() > energy_threshold)
            | (df_processed.power > min_active_load * power_capacity))
        & (df_processed.meter_number == df_processed.meter_number.shift())), 'load'] = df_processed.energy.diff()
    ...
    df_processed.loc[(
        (
            (df_processed.timestamp - df_processed.timestamp_load.shift() > pd.to_timedelta(t_between, unit='m'))
            & (df_processed.energy.diff() >= energy_threshold))
        | (df_processed.meter_number != df_processed.meter_number.shift())), 'cooking_start'] = True

    df_processed.loc[(((df_processed.energy.diff() >= energy_threshold)) & (
        (df_processed.timestamp.diff() < pd.to_timedelta(t_between, unit='m')))), 'cooking_start'] = False

    df_processed.loc[((df_processed.timestamp_load.diff() > pd.to_timedelta(t_between +
                                                                            time_resolution, unit='m')) & (df_processed.power >= min_active_load *
                                                                                                           power_capacity)), 'cooking_start'] = True

    df_processed.loc[((df_processed.cooking_start.shift(-1)) | (df_processed.meter_number !=
                                                                df_processed.meter_number.shift(-1))), 'cooking_end'] = True

    df_processed.loc[((df_processed.cooking_end.shift(-1))
    ...
    df_processed.loc[((df_processed.cooking_end.shift())
                      & (df_processed.cooking_end)), 'cooking_start'] = True

    df_processed.loc[((df_processed.cooking_start.shift())
...
def timestamp_issue(df_processed, error_margin=0.01):
    ...
    start_of_event.loc[((start_of_event['energy'] - error_margin <= start_of_event['energy'].shift())
    ...
    end_of_event.loc[((end_of_event['energy'] - error_margin <= end_of_event['energy'].shift())
    ...
    df_epc.drop(df_epc[(df_epc['timestamp_issue'] == True)].index, inplace=True)
    return df_epc
...
def only_events(
    ...
    df_only_events.loc[(df_only_events.cooking_event.diff() != 0), 'event_count'] += 1
